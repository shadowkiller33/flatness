...
def random_sampling(sentences, labels, num):
    ...
    if num > len(labels):
        assert (
            False
        ), f"you tried to randomly sample {num}, which is more than the total size of the pool {len(labels)}"
...
def load_agnews(path):
    ...
    train_labels = [
        l - 1 for l in train_labels
    ]  # make them 0, 1, 2, 3 instead of 1, 2, 3, 4
...
def load_dataset(path, params, prompt):
    ...
    if params["dataset"] == "sst2":
        ...
        params["inv_label_dict"] = {
            ...
            "Technology": 3,
            "Science": 3,
        }  # notice index start from 1 here
        params["task_format"] = "classification"
        params["num_tokens_to_predict"] = 1
        ...
        def prompt_func(
            params, train_sentences, train_labels, test_sentence, test_label_option=None
        ):
            assert seg2[-1] == " "
            prompt = ""
