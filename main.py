...
def main(models, datasets, all_shots, num_seeds, subsample_test_set, api_num_log_prob, approx, use_saved_results, bs, mode):
    ...
    if use_saved_results:
        load_results(all_params,models,mode,promptset)
    else:
        save_results(all_params,models,mode,promptset)


def save_results(params_list,models,mode, promptset ,freeze_test_set=True):
    """
    ...
    for prompt in promptset:
        difference = []
        perturbed = set()
        gpt2_model, gpt2_tokenizer = setup_gpt2(models[0])
        ids = gpt2_tokenizer.encode(prompt, return_tensors="pt", truncation=True, padding='max_length',
                               max_length=30).to('cuda')
        logits = gpt2_model(ids, return_dict=True)
        ...
        for perturbed_sentence in perturbed:
            ids1 = gpt2_tokenizer.encode(perturbed_sentence, return_tensors='pt', truncation=True, padding='max_length',
                                    max_length=30).to('cuda')
            logits1 = gpt2_model(ids1, return_dict=True)
        ...
        for param_index, params in enumerate(params_list):
            ...
            ### load data
            all_train_sentences, all_train_labels, all_test_sentences, all_test_labels = load_dataset(params,prompt+'\n\n')
            params_check(params)
            ...
            acc_original = 0#eval_accuracy(all_label_probs, test_labels)
    ...
    print('On {}, The pearson correlation between {} flatness and human score is {}'.format(models[0],mode, a))
    print('On {}, The spearman correlation between {} flatness and human score is {}'.format(models[0],mode, b))
    print('On {}, The kendall correlation between {} flatness and human score is {}'.format(models[0],mode, c))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', dest='mode', action='store', required=True, default='max',
    ...
    promptset = ['What label best describes this news article, World, Sports, Business, or Technology?', 'What is this a piece of news regarding for, World, Sports, Business, or Technology?',
     'What is the category of the following news, , World, Sports, Business, or Technology', 'Which is the most relevant topic of the following news, , World, Sports, Business, or Technology?',
     'Give the topic of the given text: World, Sports, Business, or Technology.', 'Read the text below, provide its focused topic: World, Sports, Business, or Technology.',
     'Is this a piece of news regarding World, Sports, Business, and Technology.?',
     'Which section of a newspaper would this article likely appear in, World, Sports, Business, or Technology?','Classify the news articles into the categories of World, Sports, Business, and Technology.']
    main(**args)